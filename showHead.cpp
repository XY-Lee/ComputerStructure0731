...
bool ChangeFace( CvSeq *faces, Img &mainImg, int faceNum, int headIndex ) { 
  ...
  resize( gAniHead[ headIndex ].img, reScaleTemp, Size( rectangle->***, rectangle->*** ) ) ; // make No.headIndex head's size to rectangle's size
  ...
  imgROI = mainImg.img( Rect( point1.***, point1.***, reScaleTemp.cols, reScaleTemp.rows ) );
